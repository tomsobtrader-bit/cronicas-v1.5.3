...
// ESTE ES EL MÉTODO _renderRow COMPLETO con los cambios aplicados.
    // Reemplazá el _renderRow completo en combatsystem.js con esto.
    ...
    _renderRow(side, row, selId, healMode, spellMode, sacrificeAllyMode, sacrificeEnemyMode, placingCard, guardianMode) {
        ...
        <div class="board-row">
            ...
            ${cells.map((troop, col) => {
                ...
                // ── Nuevas clases de estado de turno (solo lado jugador) ──
                const hasAttackedClass   = troop && side === "player" && troop.hasAttacked && !troop.summonFatigue
                    ? "cell-has-attacked" : ""
                const hasFatigueClass    = troop && side === "player" && troop.summonFatigue
                    ? "cell-has-fatigue" : ""
                const readyToAttackClass = troop && side === "player" && !troop.hasAttacked && !troop.summonFatigue && !troop.isStunned && troop.canAttack()
                    ? "cell-ready-to-attack" : ""
                    ...
                    ${troop ? `
                        <div class="troop-overlay">
                            ...
                            <div class="troop-tags">
                                ${troop.summonFatigue
                                    ? '<span class="tag tag-fatigue" title="Fatiga de invocación — no puede atacar este turno">⏳ NUEVO</span>'
                                    : ""}
                                ${troop.isStunned
                                    ? '<span class="tag tag-stun" title="Aturdido — pierde su próximo ataque">⚡ ATURD.</span>'
                                    : ""}
                                ${troop.poisonTurnsLeft > 0
                                    ? `<span class="tag tag-poison">☠ ${troop.poisonTurnsLeft}t</span>`
                                    : ""}
                                ${troop.burnTurnsLeft > 0
                                    ? `<span class="tag tag-burn">🔥 ${troop.burnTurnsLeft}t</span>`
                                    : ""}
                                ${troop.curseTurnsLeft > 0
                                    ? `<span class="tag tag-curse">💜 ${troop.curseTurnsLeft}t</span>`
                                    : ""}
                                ${troop.isMarked
                                    ? '<span class="tag tag-mark">🎯</span>'
                                    : ""}
                                ${troop.effect?.type === "lastStand" && troop.effect?.used
                                    ? '<span class="tag tag-used" title="Último Bastión ya usado">💔</span>'
                                    : ""}
                                ${troop.hasAttacked && !troop.summonFatigue && !troop.isStunned
                                    ? '<span class="tag tag-attacked" title="Ya atacó este turno">✔ ATACÓ</span>'
                                    : ""}
                            </div>
                        </div>
                    ` : isPlaceable
